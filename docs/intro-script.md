...
| --- | --- | --- |
| 0–~0.4 s | **Two big orbs** (a cool + a warm twin) **twirl together** — an accelerating inspiral — while a few much-bigger bodies + small **spiral in** and **hundreds of dust** swirl inward. Colourful. | Static `#osp-splash` markup (orbs/shock/plume/jet/flash/core) + a tiny **inline script**: the few CSS bodies, and a **canvas particle field** for the dust (one layer, sprite `drawImage` — cheap where hundreds of DOM nodes are not). All synchronous, so it paints on first paint. Orb keyframes rotate < 180°/step so the inspiral interpolates the same in every browser (a WebKit matrix-decomposition quirk made them briefly fly apart). |
...
| ~0.45–0.9 s | The dust **bursts back outward** as a colourful cloud (some fully saturated), **reverberating shock rings** (magenta · orange · cyan · pink, staggered, tilted) expand, and the dark **event horizon** then **grows back outward** (overshoot → settle) to the final circle + a cool photon ring (`--core-d ≈ 28vmin`, to match the real shadow). | Canvas burst phase + `.osp-splash__shock` (`--sd`/`--rc`) + `.osp-splash__core` (grows late). |
